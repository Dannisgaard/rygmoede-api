...
async def get_meetings(db: AsyncIOMotorClient = Depends(get_database), skip: int = 0, limit: int = 20): # type: ignore
    dbmeetings = await get_all_meetings(db, skip=skip, limit=limit) # Fetch first 20 meetings
